<nav x-data="{ open: false }" class="border-b border-gray-600">
        ...
        <div class="flex justify-between h-16">
            ...
            <!-- Navigation Links -->
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    ...
                    <x-nav.nav-link :href="route('genres.index')"
                    ...
                    @if(!auth()->user()->hasRole('astronaut'))
                        <x-nav.nav-link :href="route('users.index')"
                                        :active="request()->routeIs('users*')">
                            {{__('Users')}}
                        </x-nav.nav-link>
                    @endif
                    @if(auth()->user()->hasRole('admin'))
                        <x-nav.nav-link :href="route('roles.index')"
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button
                            class=" text-primary-content flex items-center text-sm font-medium hover:text-secondary-content hover:border-secondary focus:outline-none focus:text-secondary-content focus:border-secondary-content transition duration-150 ease-in-out">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden ">
        <div class="pt-2 pb-3 space-y-1 ">
            ...
            <x-responsive-nav-link :href="route('playlists.index')"
                            :active="request()->routeIs('playlists*')">
                {{__('Playlists')}}
            </x-responsive-nav-link>
            @if(!auth()->user()->hasRole('astronaut'))
